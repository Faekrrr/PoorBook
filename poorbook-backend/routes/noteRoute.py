...
@noteRouter.post("/notes", response_model=ApiResponse,
                ...
                dependencies="Create new note using Note model.")
def insertNote(newNote: Note, repository: NoteRepository = Depends()):
    """ Creates new note in collection """
    result = repository.insert(newNote)

    if not result:
        raise ItemNotCreatedException("Note hasnt been created.")
...
@noteRouter.delete("/notes/{id}", response_model=ApiResponse,
...
@noteRouter.put("/notes/{id}",
                ...
                status_code=status.HTTP_204_NO_CONTENT)
async def updateTask(id: str, changes: Note, repository: NoteRepository = Depends()):
    """ Update note by Id """
